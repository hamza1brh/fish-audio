...
# All rights reserved.
#
# DEPRECATED: This module uses legacy quantization that is slow and broken on PyTorch 2.9+.
# Please use tools/llama/quantize_torchao.py instead:
#     python tools/llama/quantize_torchao.py --mode int8
#     python tools/llama/quantize_torchao.py --mode int4 --group-size 128
#
"""
Legacy quantization module (DEPRECATED).

This module contains the original INT8/INT4 quantization implementation that has
known issues:
- INT8: Uses naive dtype conversion in forward pass, resulting in slow inference
- INT4: Uses internal PyTorch APIs that changed in 2.9+, causing shape mismatches

For production use, please migrate to TorchAO-based quantization:
    python tools/llama/quantize_torchao.py --mode int8
    python tools/llama/quantize_torchao.py --mode int4 --group-size 128
"""
import datetime
...
warnings.warn(
    "tools.llama.quantize_legacy is deprecated and may produce slow or broken models. "
    "Use tools/llama/quantize_torchao.py instead for TorchAO-based quantization.",
    DeprecationWarning,
    stacklevel=2,
)
...
def prepare_int4_weight_and_scales_and_zeros(weight_bf16, groupsize, inner_k_tiles):
    ...
    # PyTorch 2.9+ expects uint8 input for _convert_weight_to_int4pack
